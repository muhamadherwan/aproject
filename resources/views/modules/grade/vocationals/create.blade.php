...
@section('content')
    ...
    @if(!is_null($batch))
        @if ( $batch->finished() == 1)
            <div class="alert alert-success">
                @elseif ($batch->processedJobs() >= 1)
                    <div class="alert alert-primary">
                        @else
                            <div class="alert alert-danger">
                                @endif
                                <p><h6>{{$batch->name}}</h6></p>
                                <p><b>{{ $batch->processedJobs() }}</b> rekod telah siap dari jumlah keseluruhan
                                    <b>{{ $batch->totalJobs }}</b> rekod pelajar.</p>
                                <p>Status: (<b>{{ $batch->progress() }}%</b>)</p>
                            </div>
                        @endif

                        @if(!is_null($batch2))
                            @if ( $batch2->finished() == 1)
                                <div class="alert alert-success">
                                    @elseif ($batch2->processedJobs() >= 1)
                                        <div class="alert alert-primary">
                                            @else
                                                <div class="alert alert-danger">
                                                    @endif
                                                    <p><h6>{{$batch2->name}}</h6></p>
                                                    <p><b>{{ $batch2->processedJobs() }}</b> rekod telah siap dari
                                                        jumlah keseluruhan
                                                        <b>{{ $batch2->totalJobs }}</b> rekod pelajar.</p>
                                                    <p>Status: (<b>{{ $batch2->progress() }}%</b>)</p>
                                                </div>
                                            @endif
                                                        ...
                                                        <div class="card-body">
                                                            ...
                                                            <div class="row mb-3">
                                                                <form id="user-form" class="needs-validation" novalidate
                                                                      action="{{ route('grade.vocational.store') }}"
                                                                      method="post">
                                                                    @csrf
                                                                            ...
                                                                            <div class="mb-3">
                                                                                ...
                                                                                <select id="type" name="type"
                                                                                    ...
                                                                                    @foreach ($collegeTypes as $type)
                                                                                        <option
                                                                                            {{ $type->id == 1 ? 'selected' : '' }} value="{{ $type->id }}">
                                                                                            {{ $type->parameter }}
                                                                                        </option>
                                                                                    @endforeach
                                                                                </select>
                                            ...
                                            @section('script')
                                                ...
                                                @if (!is_null($batch2) && $batch2->progress() < 100)
                                                    <script>
                                                        window.setInterval('refresh()', 2000);
                                            ...
                                            <!-- Sweet Alerts js -->
                                                <script
                                                    src="{{ URL::asset('/assets/libs/sweetalert2/sweetalert2.min.js') }}"></script>

                                                <!-- Sweet alert init js-->
                                                <script
                                                    src="{{ URL::asset('/assets/js/pages/sweet-alerts.init.js') }}"></script>

                                                <!-- Required datatable js -->
                                                <script
                                                    src="{{ URL::asset('/assets/libs/datatables/datatables.min.js') }}"></script>
                                                <script
                                                    src="{{ URL::asset('/assets/libs/jszip/jszip.min.js') }}"></script>
                                                <script
                                                    src="{{ URL::asset('/assets/libs/pdfmake/pdfmake.min.js') }}"></script>

                                                <!-- Datatable init js -->
                                                <script
                                                    src="{{ URL::asset('/assets/js/pages/datatables.init.js') }}"></script>

                                                <script
                                                    src="{{ asset('/assets/js/datatables/slip-results.js') }}"></script>
@endsection
